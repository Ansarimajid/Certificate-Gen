...
def track(request, id, slug):
	...
	if request.method == "POST":
		...
		while i < df_len[0]:
			...
			# r = requests.get(f"https://docs.google.com/presentation/d/{f_id}/download/pdf", allow_redirects=True)
			...
			headers = {
				'Authorization': 'Bearer pdf_live_jtuRNKnuBKxi3FwPVAshDmh4PC1bwScdx9bFbePkCzn'
			},
			files = {
				'certificate': open('certificate.pptx', 'rb')
			},
			data = {
			...
			if response.ok:
				with open('certificate.pdf', 'wb') as fd:
